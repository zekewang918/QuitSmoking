	...
	x.getJSON(url+'&_makehtml&r='+(+ new Date()), function(ret){
		...
		if(ret && (ret=ret['data']) && ret['status'] == 'html_ok') {
			if(obj) {
				...
				obj.innerHTML = '<a href="'+ret['path']+'" target="_blank">'+title+'Build succeeds</a>';
			}
			if(ret['nexturl']) {
				if(obj) {
					obj.style.color = 'green';
					obj.innerHTML = 'Build'+title+(Math.round((ret['current']/ret['count'])*100))+'%';
				}
			...
			if(obj) {
				obj.style.color = 'red';
				obj.innerHTML = title+'Build to fail';
			}
		}
	});
...
function make_html_batch(url, ids, callback, dom, single) {
	...
	if(this.single) {
		...
		progress_bar(cent);
		child.innerHTML = 'Start generating'+this.dom.getAttribute('mktitle');
		this.dom.innerHTML = '';
		this.dom.appendChild(child);
		this.make(id, child);
		this.child = child;
		var child2 = document.createElement('div');
		child2.innerHTML = '<a href="javascript:void(0);" id="mk_goon">If your browser does not respond, Please click Continue...</a>';
		...
		x.getJSON(this.url+id+'&_makehtml&r='+(+ new Date()), function(ret){
			if(ret && (data=ret['data']) && data['status'] == 'html_ok') {
				...
				if(!makehtml_error) {
					...
					makehtml_error.innerHTML = 'Error message';
			...
			if(obj.single) {
				...
				child.innerHTML = '<div class="mk_msg">'+'<a href="'+data['path']+'" target="_blank">'+obj.dom.getAttribute('mktitle')+'</a>Generation is complete'+'</div>';
				if(obj.callback) {
				...
				progress_bar(cent);
				var str = 'The need to generate a total of'+obj.count+' '+obj.dom.getAttribute('mktitle')+'File, Successfully generated'+obj.makedcount+'，';
				if(cent != 100) {
					child.innerHTML = str+'Generating first'+current+', Has been completed'+cent+'%';
				} else {
					child.style.color = 'blue';
					child.innerHTML = str+obj.dom.getAttribute('mktitle')+'Generation is complete';
				}
				if(id) {
					obj.make(id, child);
