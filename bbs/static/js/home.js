...
function blogAddOption(sid, aid) {
	...
	if (newOption!=null && newOption!='') {
		...
		alert('The type name can not be empty!');
		return false;
	}
}
		...
		theImages[i].onload = function() {
			if (this.width > size) {
				...
				if (this.parentNode.tagName.toLowerCase() != 'a') {
					...
					this.title = 'Click on the picture to original size in a new window';
...
function ischeck(id, prefix) {
	...
	alert('Please select the object you want to operate');
	return false;
}
...
function insertWebImg(obj) {
	if(checkImage(obj.value)) {
		...
		alert('Image address is not correct');
	}
}
...
function showFlash(host, flashvar, obj, shareid) {
	...
	var videoMp3 = true;
	if('' == flashvar) {
		alert('Music wrong address, can not be empty');
		return false;
	}
	if('music' == host) {
	...
	if(flashAddr[host]) {
		...
		var nodetxt = document.createTextNode("Collapse");
...
function userapp_close() {
	...
	x.get('home.php?mod=spacecp&ac=common&op=getuserapp&subop=off&inajax=1', function(s){
		...
		$('a_app_more').innerHTML = 'Expansion';
		$('a_app_more').onclick = function() {
			userapp_open();
		};
	});
...
function myfriend_post(uid) {
	if($('friend_'+uid)) {
		$('friend_'+uid).innerHTML = '<p>You are now friends, the next, you can also: <a href="home.php?mod=space&do=wall&uid='+uid+'" class="xi2" target="_blank">To leave a message</a>, Or <a href="home.php?mod=spacecp&ac=poke&op=send&uid='+uid+'&handlekey=propokehk_'+uid+'" id="a_poke_'+uid+'" class="xi2" onclick="showWindow(this.id, this.href, \'get\', 0, {\'ctrlid\':this.id,\'pos\':\'13\'});">Say hello</a></p>';
	}
	showCreditPrompt();
}
...
function feed_more_show(feedid) {
	...
	$(opid).innerHTML = '&laquo; Collapse list';
	$(opid).onclick = function() {
...
function feed_more_close(feedid) {
	...
	$(opid).innerHTML = '&raquo; More dynamic';
	$(opid).onclick = function() {
