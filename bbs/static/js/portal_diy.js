...
drag.extend({
	'getBlocksTimer' : '',
	'blocks' : [],
	'blockDefaultClass' : [{'key':'Select a Style','value':''},{'key':'Without border and Borderless','value':'cl_block_bm'},{'key':'Style 1','value':'xbs_1'},{'key':'Style 2','value':'xbs xbs_2'},{'key':'Style 3','value':'xbs xbs_3'},{'key':'Style 4','value':'xbs xbs_4'},{'key':'Style 5','value':'xbs xbs_5'},{'key':'Style 6','value':'xbs xbs_6'},{'key':'Style 7','value':'xbs xbs_7'}],
	'frameDefaultClass' : [{'key':'Select a Style','value':''},{'key':'Without border and Borderless','value':'cl_frame_bm'},{'key':'No a border framework','value':'xfs xfs_nbd'},{'key':'Style 1','value':'xfs xfs_1'},{'key':'Style 2','value':'xfs xfs_2'},{'key':'Style 3','value':'xfs xfs_3'},{'key':'Style 4','value':'xfs xfs_4'},{'key':'Style 5','value':'xfs xfs_5'}],
	setDefalutMenu : function () {
		...
		this.addMenu('default', 'Del', 'drag.removeBlock(event)');
	...
	openStyleEdit : function (e) {
		...
		var repeatarr = [{'key':'Tile','value':'repeat'},{'key':'Tile is not','value':'no-repeat'},{'key':'Tile Horizontally','value':'repeat-x'},{'key':'Tile Vertically','value':'repeat-y'}];
		var stylearr = [{'key':'No Style','value':'none'},{'key':'Solid line','value':'solid'},{'key':'Dot line','value':'dotted'},{'key':'Dotted line','value':'dashed'}];
		var table = '<table class="tfm">';
		table += '<tr><th>Font</th><td><input type="text" id="fontsize" class="px p_fre vm" value="'+fontsize+'" size="2" />px <input type="text" id="fontcolor" class="px p_fre vm" value="'+fontcolor+'" size="2" />';
		...
		table += '<tr><th>A border</th><td><ul id="borderul" class="'+ulclass+'">';
		table += '<li><label>上</label><select class="ps vm" id="bdtwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdtwidth)+'</select>';
		...
		table += '<li class="bordera mtn"><label>Under</label><select class="ps vm" id="bdbwidth" ><option value="">Size</option>'+this.getOption(widtharr,bdbwidth)+'</select>';
		...
		table += '</ul><p class="ptm"><label><input id="borderop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'borderul\').className = $(\'borderul\').className == \'borderul\' ? \'borderula\' : \'borderul\'">Respectively provided</label></p></td></tr>';
		...
		table += '<tr><th>Outside Edge distance</th><td><div id="margindiv" class="'+ulclass+'"><span><label>Up</label> <input type="text" id="margint" class="px p_fre vm" value="'+margint+'" size="1"/>px </span>';
		table += '<span class="bordera"><label>Right</label> <input type="text" id="marginr" class="px p_fre vm" value="'+marginr+'" size="1" />px </span>';
		table += '<span class="bordera"><label>Under</label> <input type="text" id="marginb" class="px p_fre vm" value="'+marginb+'" size="1" />px </span>';
		table += '<span class="bordera"><label>Left</label> <input type="text" id="marginl" class="px p_fre vm" value="'+marginl+'" size="1" />px</span>';
		table += '</div><p class="ptm"><label><input id="marginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'margindiv\').className = $(\'margindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'">Respectively provided</label></p></td></tr>';
		...
		if (objType == 1) {
			...
			table += '<tr><th>Inside Edge distance</th><td><div id="cmargindiv" class="'+ulclass+'"><span><label>Up</label> <input class="px p_fre" id="cmargint" value="'+cmargint+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Right</label> <input class="px p_fre" id="cmarginr" value="'+cmarginr+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Under</label> <input class="px p_fre" id="cmarginb" value="'+cmarginb+'" size="1" />px </span>';
			table += '<span class="bordera"><label>Left</label> <input class="px p_fre" id="cmarginl" value="'+cmarginl+'" size="1" />px </span>';
			table += '</div><p class="ptm"><label><input id="cmarginop" type="checkbox" value="1" class="pc"'+opchecked+' onclick="$(\'cmargindiv\').className = $(\'cmargindiv\').className == \'borderul\' ? \'borderula\' : \'borderul\'"> Respectively provided</label></p></td></tr>';
		}
		table += '<tr><th>Background color</th><td><input type="text" id="bgcolor" class="px p_fre vm" value="'+bgcolor+'" size="4" />';
		table += getColorPalette(id+'_bgcPalette', 'bgcolor' ,bgcolor)+'</td></tr>';
		table += '<tr><th>Background image</th><td><input type="text" id="bgimage" class="px p_fre vm" value="'+bgimage+'" size="25" /> <select class="ps vm" id="bgrepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		var classarr = objType == 1 ? this.blockDefaultClass : this.frameDefaultClass;
		table += '<tr><th>Designation class</th><td><input type="text" id="diyClassName" class="px p_fre" value="'+diyClassName+'" size="8" /> <select class="ps vm" id="bgrepeat" onchange="$(\'diyClassName\').value=this.value;" >'+this.getOption(classarr, diyClassName)+'</select></td></tr>';
		table += '</table>';
	...
	openTitleEdit : function (e) {
		...
		common += '<tr><th>Background image:</th><td><input type="text" id="titleBgImage" class="px p_fre" value="'+bgimage+'" /> <select class="ps vm" id="titleBgRepeat" >'+this.getOption(repeatarr,bgrepeat)+'</select></td></tr>';
		common += '<tr><th>Background color:</th><td><input type="text" id="titleBgColor" class="px p_fre" value="'+bgcolor+'" size="7" />';
		common += getColorPalette(fid+'bgPalette_0', 'titleBgColor' ,bgcolor)+'</td></tr>';
		if (obj instanceof Tab) {
			var switchArr = [{'key':'Click','value':'click'},{'key':'Glide','value':'mouseover'}];
			var switchType = obj.titles['switchType'] ? obj.titles['switchType'][0] : 'click';
			common += '<tr><th>Switch Type:</th><td><select class="ps" id="switchType" >'+this.getOption(switchArr,switchType)+'</select></td></tr>';
		}
		...
		li += '<tr><th>Location:</th><td><select id="titleFloat_0" class="ps vm"><option value="" `left`>Left</option><option value="right" `right`>Right</option></select>';
		li += '&nbsp;&nbsp;Offset of: <input type="text" id="titleMargin_0" class="px p_fre vm" value="`margin`" size="2" />px</td></tr>';
	...
	removeBlock : function (e, flag) {
		...
		var obj = this.getObjByName(id);
		if (!flag) {
			if (!confirm('Are you sure you want to delete it, can not be recovered after deletion')) return false;
		}
		if (obj instanceof Block) {
			this.delBlock(id);
		} else if (obj instanceof Frame) {
	...
	blockForceUpdate : function (e,all) {
		...
		bcontent.innerHTML = '<center>Loading Content...</center>';
		var x = new Ajax();
		x.get('portal.php?mod=portalcp&ac=block&op=getblock&forceupdate=1&inajax=1&bid='+bid+'&tpl='+document.diyform.template.value, function(s) {
			if(s.indexOf('errorhandle_') != -1) {
				...
				runslideshow();
				showDialog('Sorry, you do not have permission to add or edit module', 'alert');
				doane();
			} else {
	...
	frameExport : function (e) {
		var flag = true;
		if (drag.isChange) {
			flag = confirm('You have done modified, Please save and then do export. Otherwise, the exported data will not include you of the changes made.');
		}
		if (flag) {
			if ( typeof e == 'object') {
				e = Util.event(e);
	...
	endBlockForceUpdateBatch : function () {
		if($('allupdate')) {
			$('allupdate').innerHTML = 'The operation is completed.';
	...
	getBlocks : function () {
		...
		if (this.blocks.length > 0) {
			...
			if($('allupdate')) {
				$('allupdate').innerHTML = 'Total <span style="color:blue">'+this.blocksLen+'</span> Modules are being updated first <span style="color:red">'+cur+'</span> , Has been completed <span style="color:red">'+(parseInt(cur / this.blocksLen * 100)) + '%</span>';
	...
	blockForceUpdateBatch : function (blocks) {
		...
		showDialog('<div id="allupdate" style="width:350px;line-height:28px;">To begin the update...</div>','confirm','Module data is updated', '', true, 'drag.endBlockForceUpdateBatch()');
		var wait = function() {
	...
	clearAll : function () {
		if (confirm('You sure you want to empty the page where DIY data unrecoverable empty')) {
	...
	createObj : function (e,objType,contentType) {
		if (objType == 'block' && !this.checkHasFrame()) {alert("Tip: Not found framework, please add the framework.");spaceDiy.getdiy('frame');return false;}
		e = Util.event(e);
	...
	getTabHtml : function () {
		...
		var str = '<div id="'+id+'" class="'+className+'">';
		str += '<div id="'+id+'_title" class="'+titleClassName+'"><span class="'+this.titleTextClass+'">tab tag</span></div>';
		...
		if(this.sampleMode) {
			...
			if (!this.isChange) {
				window.onbeforeunload = function() {
					return 'Your data has been modified, the exit will not be able to save your changes.';
				};
			}
			this.isChange = true;
	...
	goonDIY : function () {
		if ($('prefile').value == '1') {
			showDialog('<div style="line-height:28px;">Press the Continue button will open a temporary data and DIY, <br />Press the Delete button will delete the temporary data.</div>','confirm','Whether to continue the temporary data DIY?', function(){location.replace(location.href+'&preview=yes');}, true, 'spaceDiy.cancelDIY()', '', 'Continue', 'Del');
...
spaceDiy.extend({
		...
		if (!optype) {
			if (['portal/portal_topic_content', 'portal/list', 'portal/view'].indexOf(tplpre[0]) == -1) {
				if (document.diyform.template.value.indexOf(':') > -1 && !document.selectsave) {
					...
					showDialog('<form name="selectsave" action="" method="get"><label><input type="radio" value="0" name="savemod"'+schecked+' />Apply to such page</label>\n\
					<label><input type="radio" value="1" name="savemod"'+dchecked+' />This page applies only to</label></form>','notice', '', spaceDiy.save);
					return false;
				}
	...
	recover : function() {
		if (confirm('Are you sure you want to revert to the previous version saved it?')) {
			drag.clearClose();
