...
function toggleSettingShow() {
	if(!$('tbody_setting').style.display) {
		...
		$('a_setting_show').innerHTML = 'Collapse setting items';
	}
	doane();
}
...
function checkblockname(form) {
	if(!(trim(form.name.value) > '')) {
		showDialog('The module identification can not be empty', 'error', null, function(){form.name.focus();});
		return false;
	}
	if(form.summary && form.summary.value) {
		...
		if(tag) {
			showBlockSummary();
			form.summary.focus();
			showDialog('Custom content errors, HTML code: '+tag+' Label does not match', 'error', null, function(){form.summary.select();});
			return false;
		}
	}
		...
		for(var i = 0; i < tags.length; i++) {
			...
			if((!preTag && endTag) || (preTag && !endTag) || preTag.length != endTag.length) {
				if(returnValue) {
					...
					showDialog('HTML code: '+tag+' Label does not match', 'error', null, fn, true, fn);
					return false;
				}
			}
		}
...
function blockconver(ele,bid) {
	if(ele && bid) {
		if(confirm('Are you sure you want to convert module type from '+ele.options[0].innerHTML+' to '+ele.options[ele.selectedIndex].innerHTML)) {
...
function strLenCalc(obj, checklen, maxlen) {
	...
	if(curlen <= maxlen) {
		checklen.innerHTML = 'Has been entered <b>'+(curlen)+'</b> Characters';
		return true;
	} else {
		checklen.innerHTML = 'Exceed <b style="color:red">'+(curlen - maxlen)+'</b> Characters';
		return false;
	}
}
...
function check_itemdata_lentgh(form) {
	if(form.title && (!strLenCalc(form.title, "titlechk", form.title.getAttribute('_maxlength')) || !form.title.value)) {
		form.title.focus();
		showDialog('Title length is incorrect', 'error', null, function(){form.title.select();});
		return false;
	}
	if(form.summary && !strLenCalc(form.summary, "summarychk", form.summary.getAttribute('_maxlength'))) {
		form.summary.focus();
		showDialog('Introduction length is incorrect', 'error', null, function(){form.summary.select();});
		return false;
	}
	return true;
}