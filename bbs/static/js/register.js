...
function checkPwdComplexity(firstObj, secondObj, modify) {
	...
	firstObj.onblur = function () {
		if(firstObj.value == '') {
			...
			if(pwlength > 0) {
				pwmsg += ', Minimum length '+pwlength+' Characters';
			}
	...
	firstObj.onkeyup = function () {
		if(pwlength == 0 || $(firstObj.id).value.length >= pwlength) {
			var passlevels = new Array('','Weak','Mid','Hight');
	...
	secondObj.onblur = function () {
		if(secondObj.value == '') {
			errormessage(secondObj.id, !modifypwd ? 'Enter the password again' : profileTips);
		}
		checkpassword(firstObj.id, secondObj.id);
	};
}
...
function addMailEvent(mailObj) {
	...
	mailObj.onblur = function () {
		if(mailObj.value == '') {
			errormessage(mailObj.id, 'Please enter email address');
		}
...
var emailMenuST = null, emailMenui = 0, emaildomains = ['yahoo.com', 'gmail.com', 'hotmail.com'];
...
function checkusername(id) {
	...
	if(username.match(/<|"/ig)) {
		errormessage(id, 'User name containing sensitive characters');
		return;
	}
	var unlen = username.replace(/[^\x00-\xff]/g, "**").length;
	if(unlen < 3 || unlen > 15) {
		errormessage(id, unlen < 3 ? 'The user name must not be less than three characters' : 'he user name must not exceed 15 characters');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkpassword(id1, id2) {
	...
	if(pwlength > 0) {
		if($(id1).value.length < pwlength) {
			errormessage(id1, 'Password is too short, Not less than '+pwlength+' Characters');
			return;
		}
	}
	if(strongpw) {
		...
		for(var i in strongpw) {
			...
			if(strongpw[i] === 3 && !$(id1).value.match(/[A-Z]+/g)) {
				strongpw_error = true;
				strongpw_str[j] = 'Uppercase letters';
				j++;
			}
	...
	errormessage(id2);
	if($(id1).value != $(id2).value) {
		errormessage(id2, 'Two passwords are inconsistent');
	} else {
...
function checkemail(id) {
	...
	if(email.match(/<|"/ig)) {
		errormessage(id, 'Email Contain sensitive characters');
		return;
	}
	var x = new Ajax();
	$('tip_' + id).parentNode.className = $('tip_' + id).parentNode.className.replace(/ p_right/, '');
...
function checkinvite() {
	...
	if(invitecode.match(/<|"/ig)) {
		errormessage('invitecode', 'Invitation code containing sensitive characters');
		return;
	}
	var x = new Ajax();
