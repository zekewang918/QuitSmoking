	...
	for(var i = 0; i < buttons.length; i++) {
		if(buttons[i].id.indexOf(editorid + '_') != -1) {
			...
			if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'fullswitcher') {
				buttons[i].innerHTML = !editorisfull ? 'Full' : 'Return';
				buttons[i].onmouseover = function(e) {setEditorTip(editorisfull ? 'Restore the editor size' : 'Full screen editing');};
				buttons[i].onclick = function(e) {editorfull();doane();}
			} else if(buttons[i].id.substr(buttons[i].id.indexOf('_') + 1) == 'simple') {
				buttons[i].innerHTML = !simplodemode ? 'Commonly' : 'Advanced';
...
function savedataTime() {
	if(!autosave) {
		$(editorid + '_svdsecond').innerHTML = '<a title="Click open auto saved" href="javascript:;" onclick="setAutosave()">Click open auto saved</a> ';
		return;
	}
	if(!savedatac) {
		savedatac = savedataInterval;
		...
		h = h < 10 ? '0' + h : h;
		m = m < 10 ? '0' + m : m;
		setEditorTip('Data was ' + h + ':' + m + ' Save');
	}
	$(editorid + '_svdsecond').innerHTML = '<a title="Click Close to auto save" href="javascript:;" onclick="setAutosave()">' + savedatac + ' Seconds after the save</a> ';
	savedatac -= 10;
}

function setAutosave() {
	autosave = !autosave;
	setEditorTip(autosave ? 'Data is auto saved on' : 'Data is auto saved off');
...
function editorfull(op) {
	...
	$(editorid + '_fullswitcher').innerHTML = editorisfull ? 'Return' : 'Full';
	initesbar();
}
...
function editorsimple() {
	if($(editorid + '_body').className == 'edt') {
		...
		v = '';
		$(editorid + '_simple').innerHTML = 'Commonly';
...
function checklength(theform) {
	...
	showDialog('Current length: ' + mb_strlen(message) + ' Byte，' + (postmaxchars != 0 ? 'System limits: ' + postminchars + ' to ' + postmaxchars + ' Byte.' : ''), 'notice', 'Words checks');
}
...
function discuzcode(cmd, arg) {
	...
	if(in_array(cmd, ['sml', 'inserthorizontalrule', 'url', 'quote', 'code', 'free', 'hide', 'aud', 'vid', 'fls', 'beginning', 'attach', 'image', 'pasteword', 'index', 'postbg', 'password']) || typeof EXTRAFUNC['showEditorMenu'][cmd] != 'undefined' || cmd == 'tbl' || in_array(cmd, ['fontname', 'fontsize', 'forecolor', 'backcolor']) && !arg) {
		...
		if(txt = getSel()) {
			...
			while(listvalue = prompt('Enter a list of projects.\r\nBlank or click Cancel to finish this list.', '')) {
				if(BROWSER.opera > 8) {
					listvalue = '\n' + '[*]' + listvalue;
		...
		if(confirm('Are you sure you want to clear all do?')) {
			clearContent();
		}
	} else if(cmd == 'downremoteimg') {
		showDialog('<div id="remotedowninfo"><p class="mbn">Downloading remote attach, Please wait……</p><p><img src="' + STATICURL + 'image/common/uploading.gif" alt="" /></p></div>', 'notice', '', null, 1);
		...
		switch(tag) {
			case 'url':
				str = 'Please enter the link address:<br /><input type="text" id="' + ctrlid + '_param_1" style="width: 98%" value="" class="px" />'+
					(selection ? '' : '<br />Please enter link text:<br /><input type="text" id="' + ctrlid + '_param_2" style="width: 98%" value="" class="px" />');
				break;
			case 'forecolor':
			...
			case 'postbg':
				...
				break;
			case 'password':
				str = '<p class="pbn">Please enter Posts password: <input type="text" id="' + ctrlid + '_param_1" size="10" value="" class="px" /></p>';
				break;
			case 'code':
				if(wysiwyg) {
			...
			case 'free':
				...
				var lang = {'quote' : 'Please enter to insert references', 'code' : 'Please enter the code to insert', 'hide' : 'Please enter the info you want to hide the contents of', 'free' : 'If you set the post Price, Please enter the info content of visible before buying free'};
				str += lang[tag] + ':<br /><textarea id="' + ctrlid + '_param_1" style="width: 98%" cols="50" rows="5" class="txtarea"></textarea>' +
					(tag == 'hide' ? '<br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_1" class="pc" checked="checked" />Only when the only surfers Reply, Will be displayed</label><br /><label><input type="radio" name="' + ctrlid + '_radio" id="' + ctrlid + '_radio_2" class="pc" />Only when Credit more than</label> <input type="text" size="3" id="' + ctrlid + '_param_2" class="px pxs" /> Show only<br /><br /><label>valid days:</label> <input type="text" size="3" id="' + ctrlid + '_param_3" class="px pxs" /> <br />Greater than the number of days from the Posting Date tab automatically lapse' : '');
				break;
			case 'tbl':
				str = '<p class="pbn">Table rows: <input type="text" id="' + ctrlid + '_param_1" size="2" value="2" class="px" /> &nbsp; Table columns: <input type="text" id="' + ctrlid + '_param_2" size="2" value="2" class="px" /></p><p class="pbn">Table width: <input type="text" id="' + ctrlid + '_param_3" size="2" value="" class="px" /> &nbsp; Background color: <input type="text" id="' + ctrlid + '_param_4" size="2" class="px" onclick="showColorBox(this.id, 2)" /></p><p class="xg2 pbn" style="cursor:pointer" onclick="showDialog($(\'tbltips_msg\').innerHTML, \'notice\', \'Small Tip\', null, 0)"><img id="tbltips" title="Small Tip" class="vm" src="' + IMGDIR + '/info_small.gif"> Rapid writing table tips</p>';
				str += '<div id="tbltips_msg" style="display: none">“[tr=color]” Definition line background<br />“[td=Width]” Defined column width<br />“[td=Column span and span the width]” Define the rows and columns span<br /><br />Rapid writing table example: <div class=\'xs0\' style=\'margin:0 5px\'>[table]<br />Name:|Discuz!<br />Version:|X1<br />[/table]</div>With "|" to separate each column, Form if have "|" use "\\|" instead of, line feed "\\n" instead.</div>';
				break;
			case 'aud':
				str = '<p class="pbn">Please enter the music file address:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="xg2 pbn">Support WMA MP3 RA RM etc. Music format<br />Example: http://server/audio.wma</p>';
				break;
			case 'vid':
				str = '<p class="pbn">Please enter video address:</p><p class="pbn"><input type="text" value="" id="' + ctrlid + '_param_1" style="width: 220px;" class="px" /></p><p class="pbn">Wide: <input id="' + ctrlid + '_param_2" size="5" value="500" class="px" /> &nbsp; High: <input id="' + ctrlid + '_param_3" size="5" value="375" class="px" /></p><p class="xg2 pbn">Support Youku, potatoes, 56, cool 6 video etc. URL of the video station<br />Support wmv avi rmvb mov swf flv etc. video format<br />Example: http://server/movie.wmv</p>';
				break;
			case 'fls':
				str = '<p class="pbn">Please enter the Flash file address:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p><p class="pbn">Wide: <input id="' + ctrlid + '_param_2" size="5" value="" class="px" /> &nbsp; High: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /></p><p class="xg2 pbn">Support SWF FLV Flash etc. URL<br />Example: http://server/flash.swf</p>';
				break;
			case 'beginning':
				str = '<p class="pbn">Please enter the beginning of the animation Flash or image address:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_1" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">Click on the link address:</p><p class="pbn"><input type="text" id="' + ctrlid + '_param_2" class="px" value="" style="width: 220px;" /></p>';
				str += '<p class="pbn">Wide: <input id="' + ctrlid + '_param_3" size="5" value="" class="px" /> &nbsp; High: <input id="' + ctrlid + '_param_4" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">Stay Seconds: <input id="' + ctrlid + '_param_8" size="5" value="" class="px" /></p>';
				str += '<p class="pbn">Loading and disappear effect: </p><p class="pbn"><input id="' + ctrlid + '_param_7" type="radio" name="effect" checked />None &nbsp; <input id="' + ctrlid + '_param_5" type="radio" name="effect" />Fade in and out &nbsp; <input id="' + ctrlid + '_param_6" type="radio" name="effect" />Expand closed</p>';
				str += '<p class="xg2 pbn">Support swf flv jpg gif png URL<br />Wide high range: Wide 400~1024 High 300~640<br />Example: http://server/flash.swf</p>';
				break;
			case 'pasteword':
				stitle = 'Paste content from Word';
				str = '<p class="px" style="height:300px"><iframe id="' + ctrlid + '_param_1" frameborder="0" style="width:100%;height:100%" onload="this.contentWindow.document.body.style.width=\'550px\';this.contentWindow.document.body.contentEditable=true;this.contentWindow.document.body.focus();this.onload=null"></iframe></p><p class="xg2 pbn">By shortcut keys (Ctrl + V) to paste the contents of the Word document to the top</p>';
				menuwidth = 600;
				menupos = '00';
				menutype = 'win';
				break;
			case 'index':
				stitle = 'Create a post directory';
				str = '<p class="pbn">[index]<br />\n\
					[#<span class="xi1">Page number</span>]<span class="xi1">Title</span> &nbsp;&nbsp;<span class="xg1">Jump to a specific page</span><br />\n\
					<span class="xi1">*</span>[#<span class="xi1">tid,pid</span>]<span class="xi1">Title</span> &nbsp;&nbsp;<span class="xg1">Jump to specific the post</span><br />\n\
					[/index]<br />\n\
					<br />\n\
					<span class="xi1">Page number</span> &nbsp;&nbsp;<span class="xg1">Use [Page] current post pages After Page</span><br />\n\
					<span class="xi1">tid,pid</span> &nbsp;&nbsp;<span class="xg1">Posts TID and PID</span><br />\n\
					<span class="xi1">*</span> &nbsp;&nbsp;<span class="xg1">Add first line indent</span></p>';
				break;
			default:
				...
				if(!str) {
					...
					for(var i = 1; i <= params; i++) {
						if(i != params || !haveSel) {
							str += (promptlang[i - 1] ? promptlang[i - 1] : 'Please enter the ' + i + ' parameter:') + '<br /><input type="text" id="' + ctrlid + '_param_' + i + '" style="width: 98%" value="" class="px" />' + (i < params ? '<br />' : '');
						}
					}
				}
				break;
