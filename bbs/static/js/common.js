		...
		if(aj.XMLHttpRequest.readyState == 4 && aj.XMLHttpRequest.status == 200) {
			...
			if(aj.recvType == 'HTML') {
				...
				if(!aj.XMLHttpRequest.responseXML || !aj.XMLHttpRequest.responseXML.lastChild || aj.XMLHttpRequest.responseXML.lastChild.localName == 'parsererror') {
					aj.resultHandle('<a href="' + aj.targetUrl + '" target="_blank" style="color:red">Internal error and can not display this content</a>' , aj);
				} else {
...
function initSearchmenu(searchform, cloudSearchUrl) {
	...
	if (!!("placeholder" in document.createElement("input"))) {
		if(searchtxt.value == 'Please enter a search for content') {
			searchtxt.value = '';
		}
		searchtxt.placeholder = 'Please enter a search for content';
	} else {
		...
		searchtxt.onblur = function () {
			if(searchtxt.value == '' ) {
				searchtxt.value = 'Please enter a search for content';
				searchtxt.className = tclass + ' xg1';
			}
		};
	}
...
function searchFocus(obj) {
	if(obj.value == 'Please enter a search for content') {
		obj.value = '';
	}
...
function noticeTitleFlash() {
	if(NOTICETITLE.flashNumber < 5 || NOTICETITLE.flashNumber > 4 && !NOTICETITLE['State']) {
		document.title = (NOTICETITLE['State'] ? '【　　　】' : '【New reminder】') + NOTICETITLE['oldTitle'];
...
function setHomepage(sURL) {
	if(BROWSER.ie){
		...
		showDialog("Non-IE please manually Set The site browser Home", 'notice');
		doane();
	}
}
...
function toggleBlind(dom) {
	if(dom) {
		if(loadUserdata('is_blindman')) {
			...
			dom.title = 'Open auxiliary access';
		} else {
			saveUserdata('is_blindman', '1');
			dom.title = 'Close auxiliary access';
		}
	}
}
