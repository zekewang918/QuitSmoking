...
var STATUSMSG = {
	...
	'2' : 'The server limit can not upload large attach',
	'3' : 'User group limit can not upload large attach',
	'4' : 'Does not support these extensions',
	'5' : 'File type restrictions can not upload large attach',
	'6' : 'Today, you can not upload more attachments',
	'7' : 'Please select a picture file(' + imgexts + ')',
	'8' : 'Attach file can not be saved',
	'9' : 'No legitimate file was uploaded',
	'10' : 'Illegal operation',
	'11' : 'Today you no longer have to upload large attach'
};
...
function validate(theform) {
	...
	if(($('postsubmit').name != 'replysubmit' && !($('postsubmit').name == 'editsubmit' && !isfirstpost) && theform.subject.value == "") || !sortid && !special && trim(message) == "") {
		showError('Sorry, you can not enter a title or content');
		return false;
	} else if(mb_strlen(theform.subject.value) > 80) {
		showError('Your title is longer than 80 characters limit');
		return false;
	}
	...
	if(!disablepostctrl && !sortid && !special && ((postminchars != 0 && mb_strlen(message) < postminchars) || (postmaxchars != 0 && mb_strlen(message) > postmaxchars))) {
		showError('Not meet the requirements of the length of your posts.\n\nCurrent length: ' + mb_strlen(message) + ' Byte\nSystem limits: ' + postminchars + ' to ' + postmaxchars + ' Byte');
		return false;
	}
	if(UPLOADSTATUS == 0) {
		if(!confirm('You have to wait for uploading attach, make sure not upload these attach?')) {
			return false;
		}
	} else if(UPLOADSTATUS == 1) {
		showDialog('You have being uploaded attach, Please wait, Posts will be automatically published the upload is complete...', 'notice');
		AUTOPOST = 1;
		return false;
	}
	if(isfirstpost && $('adddynamic') != null && $('adddynamic').checked && $('postsave') != null && isNaN(parseInt($('postsave').value)) && ($('readperm') != null && $('readperm').value || $('price') != null && $('price').value)) {
		if(confirm('Because you set read permissions or sell posts, You to confirm also broadcast your audience to see it?') == false) {
			return false;
		}
	}
		...
		if(seccodecheck || secqaacheck) {
			...
			if(secqaacheck) {
				...
				if(chkv.indexOf('loading') != -1) {
					...
					showError('To verify Q & A error, Please re-fill');
					chk = 0;
				}
			}
			if(seccodecheck) {
...
function uploadNextAttach() {
	...
	var sizelimit = '';
	if(arr[4] == 'ban') {
		sizelimit = '(Attachment type is prohibited)';
...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog('The attachment upload is finished! Success ' + UPLOADCOMPLETE + ', Failure ' + UPLOADFAILED + ' :' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID[prefix ? 1 : 0] - 1; i++) {
			...
			if(AUTOPOST) {
				...
				showDialog('The attachment upload is finished!', 'right', null, null, 0, null, null, null, null, 3);
			}
...
function insertAttach(prefix, id) {
	...
	if(extensions != '' && (re.exec(extensions) == null || ext == '')) {
		...
		showError('Sorry, do not support the uploading of such extension attach.');
		return;
	}
	if(prefix == 'img' && imgexts.indexOf(ext) == -1) {
		reAddAttach(prefix, id);
		showError('Please select a picture file(' + imgexts + ')');
		return;
	}
...
function updateattachnum(type) {
	...
	if(num) {
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = 'Contain ' + num + (type == 'image' ? ' picture attachs' : ' Attachs');
		}
		...
		if($(editorid + '_' + type)) {
			$(editorid + '_' + type).title = type == 'image' ? 'Picture' : 'Attach';
		}
		if($(editorid + '_' + type + 'n')) {
...
function updateDownImageList(msg) {
	...
	if(msg == '') {
		showError('Sorry no remote Annex.');
	} else {
		...
		switchImagebutton('imgattachlist');$('imgattach_notice').style.display = '';
		showDialog('Remote attach download is complete!', 'right', null, null, 0, null, null, null, null, 3);
	}
}
...
function uploadWindowload() {
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
		...
		var sizelimit = '';
		if(arr[7] == 'ban') {
			sizelimit = '(Attach type is prohibited)';
...
function addpolloption() {
	if(curoptions < maxoptions) {
		...
		$('polloption_new').outerHTML = '<span>Has reached the maximum number of voting '+maxoptions+'</span>';
	}
}
...
function attachoption(type, op) {
	if(!op) {
		...
		showDialog('<div id="unusedwin" class="c altw" style="overflow:auto;height:100px;">' + $('unusedlist_' + type).innerHTML + '</div>' +
			...
			'<button onclick="attachoption(\'' + type + '\', 1);hideMenu(\'fwin_dialog\', \'dialog\')" class="pn pnc"><strong>Use</strong></button></p>', 'info', 'Unused' + (type == 'attach' ? 'Attach' : 'Picture'));
	} else if(op == 3) {
...
function getreplycredit() {
	...
	if(real_reply_credit > userextcredit) {
		$('replycredit').innerHTML = '<b class="xi1">Replies reward points total is too large('+real_reply_credit+')</b>';
	} else {
		if(have_replycredit > 0 && real_reply_credit < 0) {
			$('replycredit').innerHTML = "<font class='xi1'>Return of "+Math.abs(real_reply_credit)+"</font>";
		} else {
...
function deleteThread() {
	if(confirm('You sure you want to delete this post?') != 0){
		$('delete').value = '1';
		$('postform').submit();
	}
}
