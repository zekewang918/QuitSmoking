...
function _updatesecqaa(idhash) {
	if($('secqaa_' + idhash)) {
		...
		ajaxget('misc.php?mod=secqaa&action=update&idhash=' + idhash, 'secqaa_' + idhash, null, '', '', function() {
			secST['qaa_' + idhash] = setTimeout(function() {$('secqaa_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updatesecqaa(\''+idhash+'\')">Refresh verify Q & A</span>';}, 180000);
		});
	}
}
	...
	if(isUndefined(play)) {
		if($('seccode_' + idhash)) {
			...
			ajaxget('misc.php?mod=seccode&action=update&idhash=' + idhash, 'seccode_' + idhash, null, '', '', function() {
				secST['code_' + idhash] = setTimeout(function() {$('seccode_' + idhash).innerHTML = '<span class="xi2 cur1" onclick="updateseccode(\''+idhash+'\')">Refresh verify code</span>';}, 180000);
			});
		}
	} else {
...
function _checksec(type, idhash, showmsg, recall) {
	...
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		...
		if(s.substr(0, 7) == 'succeed') {
			...
			if(showmsg) {
				if(type == 'code') {
					showError('Verify code error, Please re-fill');
				} else if(type == 'qaa') {
					showError('Verify Q&A error，Please re-fill');
				}
				recall(0);
			}
		}
...
function _copycode(obj) {
	...
	setCopy(BROWSER.ie ? obj.innerText.replace(/\r\n\r\n/g, '\r\n') : obj.textContent, 'The code has been copied to the clipboard');
}
...
function _setCopy(text, msg){
	if(BROWSER.ie) {
		...
		if(r) {
			...
			showDialog('<div class="c"><div style="width: 200px; text-align: center;">copying to fail, Please select "Allow Access"</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="c"><div style="width: 200px; text-align: center; text-decoration:underline;">Click here to copy to the clipboard</div>' +
...
function _showselect(obj, inpid, t, rettype) {
	...
	if(!obj.id) {
		...
		s = '';
		if(!t) {
			s += showselect_row(inpid, '1 day', 1, 0, rettype);
			s += showselect_row(inpid, '1 week', 7, 0, rettype);
			s += showselect_row(inpid, '1 month', 30, 0, rettype);
			s += showselect_row(inpid, '3 month', 90, 0, rettype);
			s += showselect_row(inpid, 'Custom', -2);
		} else {
			if($(t)) {
				...
				s += showselect_row(inpid, 'Custom', -1);
			} else {
				s += '<a href="javascript:;" onclick="$(\'' + inpid + '\').value = \'0\'">永久</a>';
				s += showselect_row(inpid, '7 days', 7, 1, rettype);
				s += showselect_row(inpid, '14 days', 14, 1, rettype);
				s += showselect_row(inpid, '1 month', 30, 1, rettype);
				s += showselect_row(inpid, '3 month', 90, 1, rettype);
				s += showselect_row(inpid, '6 month', 182, 1, rettype);
				s += showselect_row(inpid, '1 year', 365, 1, rettype);
				s += showselect_row(inpid, 'Custom', -1);
			}
		}
		$(div.id).innerHTML = s;
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	if(!menu && !pn) {
		...
		menu.id = menuid;
		if(cover) {
			menu.innerHTML = '<div class="zoominner" id="' + menuid + '_zoomlayer" style="display:none"><p><span class="y"><a id="' + menuid + '_imglink" class="imglink" target="_blank" title="Open in a new window">Open in a new window</a><a id="' + menuid + '_adjust" href="javascipt:;" class="imgadjust" title="Actual size">Actual size</a>' +
				'<a href="javascript:;" onclick="hideMenu()" class="imgclose" title="Close">Close</a></span>The mouse wheel to zoom the picture</p>' +
				'<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		} else {
			menu.innerHTML = '<div class="popupmenu_popup" id="' + menuid + '_zoomlayer" style="width:auto"><span class="right y"><a href="javascript:;" onclick="hideMenu()" class="flbc" style="width:20px;margin:0 0 2px 0">Close</a></span>The mouse wheel to zoom the picture<div class="zimg_p" id="' + menuid + '_picpage"></div><div class="hm" id="' + menuid + '_img"></div></div>';
		}
	...
	if(typeof zoomgroup == 'object' && zoomgroup[obj.id] && typeof aimgcount == 'object' && aimgcount[zoomgroup[obj.id]]) {
		...
		if(authorlength > 1) {
			...
			if(authorcurrent !== '') {
				...
				picpage += ' <div id="zimg_prev" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'0 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'0 -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_prev"><strong>Previous</strong></div> ';
				paid = authorcurrent < authorlength - 1 ? authorimgs[authorcurrent + 1] : authorimgs[0];
				picpage += ' <div id="zimg_next" onmouseover="dragMenuDisabled=true;this.style.backgroundPosition=\'100% 50px\'" onmouseout="dragMenuDisabled=false;this.style.backgroundPosition=\'100% -100px\';" onclick="_zoom_page(\'' + paid + '\', ' + (showexif ? 1 : 0) + ')" class="zimg_next"><strong>Next</strong></div> ';
			}
			if(picpage) {
...
function _widthauto(obj) {
	var strs = ['Switch to Wide', 'Switch to narrow'];
	if($('css_widthauto')) {
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		...
		msg += '>Download desktop shortcut</a>' + '", The download is complete, You can move files to the system desktop<br />';
		msg += '2、Click"' + '<a href="forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
		msg += 'Download ICO icon</a>' + '", The download is complete, Right-click on the desktop shortcut file -> Properties -> Change Icon, ICO icon to select the downloaded';
		showDialog(msg, 'notice', 'Add a desktop shortcut');
	};
